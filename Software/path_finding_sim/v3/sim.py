#!/usr/bin/env python3
"""
sim.py -- RSX CanSat 2026 descent-guidance interactive simulator (v1 + replan)

Real-time front-end on the real C++ guidance (pathguidance pybind11 module):
  * Play / Pause / Reset      -- fly the path forward in time, watch the glider.
  * sliders                   -- change every initial value live (re-plans).
  * Replan now / auto-replan  -- receding-horizon re-close from the current
                                 state, using the MEASURED glide ratio.
  * DRAG the glider dot        -- inject an external push / gust, then Replan to
                                 re-close from the displaced state.

v1 guidance = LINE (homing) -> ARC (loiter) -> LINE (approach). The homing
heading is DERIVED (aim at the loiter entry), so there is no homing-heading
slider; it is shown read-only in the title. Curvature steps 0<->1/R at the
corners (G1) -- visible in the curvature panel.
...
SEGCOL = {0: "#eef3ff", 1: "#eafbef"}                # Line / Arc


def default_vals():
    return dict(land=30.0, R=40.0, gr=3.0, appr=60.0, dir=1.0, look=8.0, alt=200.0, sink=1.0)
...
class Sim:
    """Headless-testable v1 simulation. step() advances the glider one dt."""
    ...
    def reset_state(self):
        ...
        self.yaw = self.g.homing_heading() if self.feasible() else 0.0
...
def run_gui():
    vals = default_vals()
    sim = Sim(build_params(vals))

    fig = plt.figure(figsize=(14.5, 8.6))
    fig.canvas.manager.set_window_title("RSX CanSat 2026 -- v1 descent guidance")
    gs = fig.add_gridspec(2, 2, width_ratios=[1.55, 1.0], left=0.055, right=0.985,
                          top=0.95, bottom=0.36, hspace=0.34, wspace=0.20)
    ...
    (entry_pt,)   = axP.plot([], [], "s", color="#8957e5", ms=6, ls="none", zorder=7)
    (replan_pts,) = axP.plot([], [], "o", color="#2da44e", ms=6, ls="none", zorder=7)
    axP.scatter([0], [0], c="green", s=60, zorder=6)
    axP.scatter([60], [90], c="red", s=95, marker="*", zorder=6)
    ...
    axK.set_title("Curvature -- v1 steps 0<->1/R at the corners (G1)", fontsize=9)
    axK.grid(alpha=0.3)
    seg_spans = []
    ...
    def mkslider(x, y, w, lbl, lo, hi, val, fmt="%.0f"):
        return Slider(fig.add_axes([x, y, w, 0.022]), lbl, lo, hi, valinit=val, valfmt=fmt)
    cL, cR = 0.075, 0.575; w = 0.33
    s = {}
    s["land"] = mkslider(cL, 0.285, w, "land hdg (deg)", 0, 359, vals["land"])
    s["R"]    = mkslider(cL, 0.245, w, "loiter R (m)", 20, 80, vals["R"])
    s["gr"]   = mkslider(cL, 0.205, w, "glide ratio", 2.0, 6.0, vals["gr"], "%.2f")
    s["appr"] = mkslider(cL, 0.165, w, "approach len (m)", 25, 120, vals["appr"])
    s["dir"]  = mkslider(cL, 0.125, w, "loiter dir (-1/+1)", -1, 1, vals["dir"], "%+.0f")
    s["look"] = mkslider(cR, 0.285, w, "lookahead drop (m)", 2, 25, vals["look"])
    s["alt"]  = mkslider(cR, 0.245, w, "start alt (m)", 80, 400, vals["alt"])
    s["sink"] = mkslider(cR, 0.205, w, "sink bias x", 0.8, 1.4, vals["sink"], "%.2f")
    s_rep = mkslider(cR, 0.165, w, "auto-replan every (m)", 0, 60, 0.0)
    s_spd = mkslider(cR, 0.125, w, "sim speed (x)", 0.5, 6.0, 2.0, "%.1f")

    b_play  = Button(fig.add_axes([cL, 0.05, 0.10, 0.05]), "Play")
    b_reset = Button(fig.add_axes([cL + 0.12, 0.05, 0.10, 0.05]), "Reset")
    b_rep   = Button(fig.add_axes([cL + 0.24, 0.05, 0.12, 0.05]), "Replan now")
    fig.text(cR, 0.06, "Drag the blue glider dot to inject a gust / push,\nthen 'Replan now' to re-close from there.",
    ...
    def redraw_plan():
        ...
        circle_line.set_data(c.e + sim.p.loiter_radius * np.sin(th), c.n + sim.p.loiter_radius * np.cos(th))
        ent = sim.g.entry(); entry_pt.set_data([ent.e], [ent.n])
        dl = 22.0
        land_arrow.xy = (60 + dl * math.sin(sim.p.land_heading), 90 + dl * math.cos(sim.p.land_heading))
        land_arrow.set_position((60, 90))
    ...
    def redraw_state():
        ...
        t = f"v1: {sname}"
        if sim.feasible():
            t += (f" | {ph} | homing {math.degrees(sim.g.homing_heading()):.0f} deg(derived)"
                  f" | {sim.g.loiter_turns():.2f}t | alt {-sim.d:.0f} m | resid {sim.g.budget_residual():+.0f} m")
            if sim.landed:
    ...
    def on_tick():
        ...
        redraw_state()
        if sim.landed:
            stt["run"] = False; b_play.label.set_text("Play")
    timer.add_callback(on_tick); timer.start()

    def apply(_=None):
        v = {k: s[k].val for k in s}
        sim.sink_bias = v["sink"]
    ...
    def on_play(_):
        stt["run"] = not stt["run"]; b_play.label.set_text("Pause" if stt["run"] else "Play")
    b_play.on_clicked(on_play)

    def on_reset(_):
        sim.reset_state(); stt["run"] = False; b_play.label.set_text("Play"); redraw_state()
    ...
    def hit(ev):
        return (ev.inaxes is axP and ev.xdata is not None
...
def save_static():
    ...
    axP.scatter([0], [0], c="green", s=60); axP.scatter([60], [90], c="red", s=95, marker="*")
    axP.set_aspect("equal"); axP.grid(alpha=0.3); axP.legend(fontsize=8)
    axP.set_title(f"v1  homing {math.degrees(sim.g.homing_heading()):.0f} deg(derived)  |  miss {sim.miss():.1f} m"
                  f"  (install python3-tk for the interactive window)", fontsize=9)
    ss, ks = curvature_xy(sim.g); axK.plot(ss, ks, color="#8250df"); axK.grid(alpha=0.3)
    axK.set_title("Curvature (v1: steps, G1)", fontsize=9)
    axX.plot(sim.alt_log, sim.xte_log, color="#d1242f"); axX.invert_xaxis(); axX.grid(alpha=0.3)
    axX.set_title("Cross-track", fontsize=9)
    fig.savefig("path_plot_v1.png", dpi=130); print("saved path_plot_v1.png")
