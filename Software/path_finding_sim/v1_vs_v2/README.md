...
`sim.py` opens a **real-time side-by-side** window comparing v1 and v2 on the
same C++ class:
- **v1 (left)** `legacy_v1=true` — LINE→ARC→LINE, homing heading *derived* (aims
  at the loiter entry), sharp corners (curvature steps 0↔1/R, G1), continuous
  sweep so the budget closes exactly.
- **v2 (right)** — LINE→CLOTHOID→ARC→CLOTHOID→LINE, homing heading *input*,
  curvature-continuous (G2), loop-quantised budget + approach-length flex.
- Curvature panel overlays both — v1 steps, v2 ramps (the whole point).
- Play / Pause / Reset fly both in lockstep; sliders change every init value and
  re-plan both; "Replan now" / auto-replan re-close both (measured glide ratio).
- **Drag either glider dot** to inject the same gust into *both* modes, then
  Replan to compare how each recovers.

The homing-heading and transition-Lc sliders only affect v2 (v1 derives its own
heading and has no clothoid). It needs an interactive matplotlib backend; if no
window appears it saves a static comparison `path_plot_v2.png` and prints:
```
