...
struct GuidanceParams {
    ...
    // v1 legacy mode: line -> arc -> line, homing heading DERIVED (aim at the
    // loiter entry), sharp corners (curvature steps, G1 at best), continuous
    // loiter sweep (closes the budget exactly, no loop quantisation). Used for
    // side-by-side comparison against the v2 clothoid path.
...
private:
    ...
    PlanStatus buildV1FromStart(Vec2 start, float z0);   // legacy line-arc-line
    PlanStatus buildV1LoiterTail(Vec2 cur, float z0);
