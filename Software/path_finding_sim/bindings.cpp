//
// bindings.cpp  --  pybind11 wrapper (desktop-only; not compiled for STM32).
// Exposes the *actual* PathGuidance class so the Python sim tests real code.
//
...
PYBIND11_MODULE(pathguidance, m) {
    m.doc() = "RSX CanSat 2026 three-phase descent guidance";

    py::class_<Vec2>(m, "Vec2")
        .def(py::init<>())
        .def_readwrite("n", &Vec2::n)
        .def_readwrite("e", &Vec2::e)
        .def("__repr__", [](const Vec2& v){
            return "Vec2(n=" + std::to_string(v.n) + ", e=" + std::to_string(v.e) + ")"; });
    ...
    py::class_<PathGuidance>(m, "PathGuidance")
        ...
        .def("entry", &PathGuidance::entry)
        .def("exit", &PathGuidance::exit)
        .def("center", &PathGuidance::center)
        .def("loiter_sweep", &PathGuidance::loiterSweep)
        .def("loiter_turns", &PathGuidance::loiterTurns)
        .def("homing_heading", &PathGuidance::homingHeading)
        .def("d_entry", &PathGuidance::dEntry)
        .def("d_exit", &PathGuidance::dExit)
