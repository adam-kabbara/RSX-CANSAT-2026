...
class Sim:
    ...
    def reset_state(self):
        ...
        self.t = 0.0; self.t_log = []; self.events = []
    ...
    def replan_now(self):
        ...
        if str(r).split('.')[-1] in ("Ok","Degraded"):
            self.replan_marks.append((self.e,self.n)); self.events.append((self.t,'r'))
        return r
    def zshift(self, dz):
        ...
        self.events.append((self.t,'z')); self.replan_now()
    def step(self, auto_replan_m=0.0):
        ...
        self.alt_log.append(-self.d); self.xte_log.append(xte); self.t_log.append(self.t)
...
def run_gui():
    ...
    gs = fig.add_gridspec(3,2,width_ratios=[1.55,1.0],left=0.055,right=0.985,top=0.95,bottom=0.40,hspace=0.45,wspace=0.20)
    axP=fig.add_subplot(gs[:,0]); axK=fig.add_subplot(gs[0,1]); axX=fig.add_subplot(gs[1,1]); axZ=fig.add_subplot(gs[2,1])
    ...
    (alt_line,)=axZ.plot([],[],"-",color="#0a7d3c",lw=1.6)
    axZ.set_xlabel("time (s)"); axZ.set_ylabel("altitude AGL (m)")
    axZ.set_title("Altitude vs time (z-shift=orange, replan=green)",fontsize=9); axZ.grid(alpha=0.3)
    z_event_lines=[]
    ...
    def redraw_state():
        ...
        if sim.t_log:
            alt_line.set_data(sim.t_log,sim.alt_log)
            axZ.set_xlim(0,max(1.0,sim.t_log[-1])); axZ.set_ylim(0,max(10.0,max(sim.alt_log)*1.1))
            for ln in z_event_lines: ln.remove()
            z_event_lines.clear()
            for (te,kind) in sim.events:
                z_event_lines.append(axZ.axvline(te,color="#fb8500" if kind=='z' else "#2da44e",lw=1.0,ls=":"))
...
def save_static():
    ...
    while not sim.landed and sim.feasible():
        sim.step()
        if (not injected) and sim.cmd and int(sim.cmd.phase)==3 and -sim.d<140:
            sim.zshift(+50); injected=True
    fig=plt.figure(figsize=(14,8.5))
    gs=fig.add_gridspec(3,2,width_ratios=[1.55,1.0],left=0.06,right=0.98,top=0.93,bottom=0.07,hspace=0.5,wspace=0.2)
    axP=fig.add_subplot(gs[:,0]); axK=fig.add_subplot(gs[0,1]); axX=fig.add_subplot(gs[1,1]); axZ=fig.add_subplot(gs[2,1])
    h=sim.p.box_n_max; axP.add_patch(plt.Rectangle((-h,-h),2*h,2*h,fill=False,ec="0.7",ls="--"))
    rn,re_=reference_xy(sim.g); axP.plot(re_,rn,"--",color="0.6",lw=1.5,label="plan (final)")
    ...
    axP.set_title("v3 + z-shift demo  land_dir %.0f side%+d R%.0f N%d  miss %.1f m"%(
    ...
    axX.plot(sim.alt_log,sim.xte_log,color="#d1242f"); axX.invert_xaxis(); axX.grid(alpha=0.3); axX.set_title("Cross-track vs altitude",fontsize=9)
    axZ.plot(sim.t_log,sim.alt_log,color="#0a7d3c",lw=1.6)
    for (te,kind) in sim.events: axZ.axvline(te,color="#fb8500" if kind=='z' else "#2da44e",lw=1.0,ls=":")
    axZ.grid(alpha=0.3); axZ.set_title("Altitude vs time (orange=z-shift, green=replan)",fontsize=9)
    axZ.set_xlabel("time (s)"); axZ.set_ylabel("alt AGL (m)")
