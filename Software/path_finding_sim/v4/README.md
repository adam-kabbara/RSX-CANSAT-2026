...
`replan(state)` adopts the **measured glide ratio** (`|v_h|/v_d`) so the budget
closes against actual sink. Behaviour depends on where you are:
- **Established in the loiter** (detected by proximity to the current circle):
  keep the *same* spiral — same side, same location, anchored to the same
  landing — and only re-close the remaining sweep, resizing the radius minimally
  (biased to the current radius, so an on-track replan is a no-op). No Dubins
  re-entry, no relocation. A bigger z-shift is absorbed by adding/removing loops
  on that same spiral; only if it genuinely can't fit does it fall back to a full
  re-solve.
- **Before the loiter (align/homing) or on approach**: full re-solve from the
  current state (may re-pick landing direction / spiral side / radius).
