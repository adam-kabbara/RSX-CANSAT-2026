...
uint8_t BMP5_Start_Mode(BMP5 *dev, uint8_t mode, uint8_t dataRate, uint8_t osr_p, uint8_t osr_t) {
	...
	// Set OSR
    uint8_t dataReg = 0b01000000 + (osr_p << 3) + osr_t;
    if (BMP5_WriteReg(dev, BMP5_REG_OSR_CONFIG, 1, &dataReg) == 1) {
        return 1;
    }

    // Set mode & data rate
    dataReg = (dataRate << 2) | mode;
...
/**
 *  @brief Set filter coefficients for the pressure and temperature IIR filters
 *  @param iir_p: Pressure IIR LPF band filter selection.
 *  @param iir_t: Temperature IIR LPF band filter selection
 *  @retval error codes: 0 = OK; 1 = I2C_ERROR; 2 = PARAM_ERROR.
 *  @note coef_1 - 0.1147rad/s, coef_3 - 0.0459rad/s, coef_7 - 0.0212rad/s, coef_15 - 0.01025rad/s, coef_31 - 0.005041rad/s, coef_63 - 0.0025rad/s, coef_127 - 0.00125rad/s
 */
uint8_t BMP5_Set_IIR(BMP5 *dev, uint8_t iir_p, uint8_t iir_t) {
	// Param check
	if(iir_p > BMP5_IIR_COEF_127 || iir_t > BMP5_IIR_COEF_127) {
		return 2;
	}

	// Set IIR Config
	uint8_t dataReg = (iir_p << 3) + iir_t;
	if (BMP5_WriteReg(dev, BMP5_REG_DSP_IIR, 1, &dataReg) == 1) {
		return 1;
	}
...
uint8_t BMP5_WriteReg(BMP5 *dev, uint8_t reg, uint8_t size, uint8_t *data) {
	...
	HAL_StatusTypeDef status = HAL_I2C_Mem_Write(dev->i2cHandle, dev->i2c_addr, reg, I2C_MEMADD_SIZE_8BIT, data, size, BMP5_TIMEOUT_MS);
	if (status != HAL_OK) {
		return 1;
	}
	return 0;
...
uint8_t BMP5_ReadReg(BMP5 *dev, uint8_t reg, uint8_t size, uint8_t *data) {
	...
	HAL_StatusTypeDef status = HAL_I2C_Mem_Read(dev->i2cHandle, dev->i2c_addr, reg, I2C_MEMADD_SIZE_8BIT, data, size, BMP5_TIMEOUT_MS);
	if (status != HAL_OK) {
		return 1;
	}
	return 0;
